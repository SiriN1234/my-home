...
driver.find_element_by_xpath('//*[@id="schMapDiv"]/div[1]/div[1]/div[2]/span[7]/a/img').click()
...
sejong_name_list = []
sejong_re_date_list = []
sejong_pre_date_list = []
sejong_corporation_list = []
...
for i in range(10):
        ...
        sejong_name_list.append(name1)
        sejong_re_date_list.append(re_date)
        sejong_pre_date_list.append(pre_date)
        sejong_corporation_list.append(corporation)
        print(i)
    
for i in range(len(sejong_area_list)):
    sejong_area_list[i] = sejong_area_list[i].replace('\n', '')

df = pd.DataFrame((zip(sejong_title_list, sejong_status_list, sejong_area_list, sejong_name_list, sejong_re_date_list, sejong_pre_date_list, sejong_corporation_list)),    
                   columns = ['title', 'status', 'area', 'name', 're_date', 'pre_date', 'corporation'])

print(df)

df.to_csv('./region_data/Sejong_notice.csv', index = False, encoding='utf-8')
...
df.to_csv('./region_data/Sejong_url.csv', index = False, encoding='utf-8')