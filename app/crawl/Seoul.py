...
driver.find_element_by_xpath('//*[@id="schMapDiv"]/div[1]/div[1]/div[2]/span[6]/a/img').click()
...
for i in range(10):
        ...
        seoul_title_list.append(title)
        seoul_status_list.append(status)
        seoul_area_list.append(area)
        seoul_name_list.append(name1)
        seoul_re_date_list.append(re_date)
        seoul_pre_date_list.append(pre_date)
        seoul_corporation_list.append(corporation)
        print(i)
    
for i in range(len(seoul_area_list)):
    seoul_area_list[i] = seoul_area_list[i].replace('\n', '')

df = pd.DataFrame((zip(seoul_title_list, seoul_status_list, seoul_area_list, seoul_name_list, seoul_re_date_list, seoul_pre_date_list, seoul_corporation_list)),    
                   columns = ['title', 'status', 'area', 'name', 're_date', 'pre_date', 'corporation'])

print(df)

df.to_csv('./region_data/Seoul_notice.csv', index = False, encoding='utf-8')
...
df.to_csv('./region_data/Seoul_url.csv', index = False, encoding='utf-8')