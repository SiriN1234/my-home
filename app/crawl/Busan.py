...
#경로
url = 'C:/Users/human/Desktop/my-home-alarm2/notice'
...
driver.find_element_by_xpath('//*[@id="schMapDiv"]/div[1]/div[1]/div[2]/span[12]/a/img').click()
...
gangwon_area_list = []
gangwon_name_list = []
gangwon_re_date_list = []
gangwon_pre_date_list = []
gangwon_corporation_list = []
...
for i in range(10):
    ...
    gangwon_title_list.append(title)
    gangwon_status_list.append(status)
    gangwon_area_list.append(area)
    gangwon_name_list.append(name1)
    gangwon_re_date_list.append(re_date)
    gangwon_pre_date_list.append(pre_date)
    gangwon_corporation_list.append(corporation)
    print(i)
    
for i in range(len(gangwon_area_list)):
    gangwon_area_list[i] = gangwon_area_list[i].replace('\n', '')

df = pd.DataFrame((zip(gangwon_title_list, gangwon_status_list, gangwon_area_list, gangwon_name_list, gangwon_re_date_list, gangwon_pre_date_list, gangwon_corporation_list)),    
                   columns = ['title', 'status', 'area', 'name', 're_date', 'pre_date', 'corporation'])


df.to_csv('./region_data/Busan_notice.csv', index = False, encoding='utf-8')
...
df.to_csv('./region_data/Busan_url.csv', index = False, encoding='utf-8')
