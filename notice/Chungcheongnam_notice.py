...
driver.find_element_by_xpath('//*[@id="schMapDiv"]/div[1]/div[1]/div[2]/span[4]/a/img').click()
...
for i in range(10):
        ...
        re_date_list.append(re_date)
        pre_date_list.append(pre_date)
        corporation_list.append(corporation)
        print(i)
    
for i in range(len(area_list)):
    area_list[i] = area_list[i].replace('\n', '')

df = pd.DataFrame((zip(title_list, status_list, area_list, name_list, re_date_list, pre_date_list, corporation_list)),    
                   columns = ['title', 'status', 'area', 'name', 're_date', 'pre_date', 'corporation'])

print(df)

df.to_csv('./Chungcheongnam_notice.csv', index = False, encoding='utf-8')
...
df.to_csv('./Chungnam_url.csv', index = False, encoding='utf-8')